...
// 인터페이스는 public static final로 선언된 상수와 추상 메소드만 가질 수 있다.
// 그냥 변수를 선언해도 컴파일러에 의해서 public static final 자동으로 붙어서 생성된다.
// 메소드도 마찬가지로 그냥 선언해도 public abstract가 붙어서 생성된다.
// 인터페이스의 메소드는 추상 메소드이기 때문에 구현부를 만들 수 없다.
// 인터페이스도 추상 클래스의 일종이기 때문에 객체를 만들 수가 없어서 자식 객체를 사용한다.
...
//	}
	...
	// default 메소드는 인터페이스 메소드에 구현부({} 블록)까지 만들 수 있는 메소드
	// default 메소드는 구현부를 만들 수 있기 때문에 상속받은 자식 클래스에서
	// 구현하지 않아도 된다.
